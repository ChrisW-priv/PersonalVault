...
\stateAccepting[below right]{1}{3}
\arrowFromTo{0}{1}{$\epsilon$}
...
\newcommand{\arrowFromTo}[4][above]{\draw[->] (#2) to node [#1, align=center] {#4} (#3);}
\newcommand{\arrowLoop}[3][below]{\draw[->] (#2) edge [loop #1] node [align=center] {#3} (#2);}
\newcommand{\arrow}[above]{\draw[->] (#2) .. controls +(270:10mm) and +(270:10mm) .. node [above, align=center] {$\epsilon$} (#3);
}
...
\state{0}{1}
\state{1}{2}
\stateAccepting[below right]{2}{3}
\arrowFromTo{0}{1}{$\epsilon$}
\arrowFromTo{2}{1}{$\epsilon$}
\arrowFromTo{2}{3}{$\epsilon$}
\arrowFromTo{0}{3}{$\epsilon$}
}
